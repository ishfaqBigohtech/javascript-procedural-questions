...
import Commisioner from "./src/Commisioner";
...
class App {
  ...
  getConstituency() {
    return MP.constituencyInfo();
  }

  getDriver() {
    let MPDriver = MP.assignedDriver();
    if (MPDriver) return `Driver assigned to MP: ${MPDriver}`;
  }

  exceedsSpendingLimit() {
    return Commisioner.arrestOnIncreasingExpenses();
  }

  canArrest() {
    return Commisioner.canArrest();
  }
}

const result = new App();
console.log("Constituency:", result.getConstituency());
console.log(result.getDriver());
console.log(result.exceedsSpendingLimit());
result.canArrest();
